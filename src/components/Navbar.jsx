...
const Navbar = () => {
   ...
   let [click,setclick] =useState(false)
    ...
    return (
        <div className='w-full px-5 md:px-[5vw] fixed md:opacity-5 md:hover:opacity-100 py-[3vw]  overflow-hidden flex  items-center bg-transparent justify-between  z-[999] font-["NeueMontreal"]  '> 
            <div 
            
            className='logo    '>
                {click ?null:<svg width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className='Links  gap-10 hidden md:flex '>
                {['service','our work','about us','insights','contact'].map((ele,index)=>(<a key={index} className={`text-lg capitalize font-light ${index===4 && ""}`} >{ele}</a>))}
            </div>
            <div>
                <button onClick={()=>{setclick(!click)}} className='text-lg float- capitalize  md:hidden  font-light w-[20vw] h-[7vw] rounded-full border-[1px] border-x-zinc-600'>MORE

                    {click? <div  className="toggle mt-[3vw] bg-transparent rounded-lg  " >
                    {['service','our work','about us','insights','contact'].map((ele,index)=>(<a key={index} className={`text-lg block capitalize font-light  mt-[2vh]  `} >{ele}</a>))}
